...
export default function Auth() {
  ...
  async function forgot() {
    ...
    if (!parsed.success) { setMessage("Enter a valid email address."); return; }
    ...
    <div className="grid min-h-screen bg-background lg:grid-cols-2">
      ...
      <main className="flex items-center justify-center p-6">
        <div className="w-full max-w-md">
          ...
          <h1 className="mt-5 font-display text-3xl font-bold">{mode === "login" ? "Welcome back" : "Create your account"}</h1>
          <p className="mt-2 text-muted-foreground">{mode === "login" ? "Continue your learning journey." : "Start as a student. Tutor applications follow after signup."}</p>
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            <Button type="submit" size="lg" className="w-full" disabled={busy}>{busy ? "Please wait…" : mode === "login" ? "Sign in" : "Create account"}</Button>
          </form>
          ...
          {mode === "login" && <button onClick={forgot} className="mt-4 w-full text-center text-sm text-muted-foreground">Forgot password?</button>}
        </div>
      </main>
    </div>
  );
