...
import { ArrowRight, BookOpenCheck, PlayCircle, Quote, Users } from "lucide-react";
    ...
    <SiteShell>
      ...
      <section className="relative overflow-hidden bg-background">
        ...
        <div className="relative mx-auto grid min-h-[640px] max-w-7xl items-center px-4 py-24 sm:px-6 lg:min-h-[720px] lg:px-8">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }} className="max-w-2xl">
            <p className="mb-6 text-xs font-semibold uppercase tracking-[0.22em] text-primary">Cambridge · IGCSE · A-Level</p>
            <h1 className="font-display text-5xl font-semibold leading-[1.05] tracking-tight text-foreground sm:text-6xl lg:text-7xl">Catch Up.<br /><span className="text-primary">Stay Ahead.</span></h1>
            <p className="mt-6 max-w-xl text-lg leading-8 text-muted-foreground">Personalized tutoring and premium resources that help students master Mathematics with clarity and confidence.</p>
            <div className="mt-10 flex flex-wrap gap-3">
              <Button asChild size="lg" variant="hero" className="shadow-lift"><Link to="/tutors">Find a Tutor <ArrowRight /></Link></Button>
              <Button asChild size="lg" variant="outline"><Link to="/resources">Explore Resources</Link></Button>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <div className="max-w-2xl"><p className="text-sm font-bold uppercase tracking-widest text-primary">One connected learning journey</p><h2 className="mt-3 font-display text-4xl font-bold">From first question to exam confidence.</h2></div>
        <div className="mt-10 grid gap-5 md:grid-cols-3">{[[BookOpenCheck, "Practice with purpose", "Yearly papers, marking schemes, topic packs, and solutions organized around your curriculum."], [PlayCircle, "Learn without leaving", "Watch free and premium explanations inline, then move directly into focused practice."], [Users, "Get expert support", "Book approved tutors by subject, topic, availability, and transparent session pricing."]].map(([Icon, title, text], i) => { const I = Icon as typeof BookOpenCheck; return <motion.article key={String(title)} whileHover={{ y: -6 }} className="rounded-3xl border bg-card p-7 shadow-soft"><div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${i === 1 ? "bg-brand-orange/10 text-brand-orange" : i === 2 ? "bg-brand-green/15 text-brand-navy" : "bg-primary/10 text-primary"}`}><I /></div><h3 className="mt-7 font-display text-xl font-bold">{String(title)}</h3><p className="mt-3 leading-7 text-muted-foreground">{String(text)}</p></motion.article>; })}</div>
      </section>
      <Testimonials />
    </SiteShell>
  );
